...
def main():
    ...
    parser.add_argument('--save_path', default = './weights/', help='Specify where to save checkpoints during training')
    parser.add_argument('--MOT_path', default = './data',
...
def get_dataset(args):
    ...
    # For simulated images, multiply the ground truth image
    # by a value less than 1 to generate a different amplitude each time
    ...
    if 'MOTvideo' in args.data:
        ...
        dataset_path = str(Path(args.MOT_path) / 'train/*')
        print(dataset_path)
        ...
        dataset_train_video = dset.Get_sample_batch_simvideo_distributed2(
            video_dataset_dir,composed_transforms_video,crop_size = crop_size)
        dataset_test_video = dset.Get_sample_batch_simvideo_distributed2(
            video_dataset_dir_test,composed_transforms_video_test,start_ind=0,
            crop_size = crop_size)
        # add to dataset list
    ...
    if 'stills' in args.data:
        print("args.stills_path: ",args.stills_path)
        ...
        dataset_test = dset.Get_sample_batch(all_files_mat_test,
                                             composed_transforms_test, start_ind=0)
...
def train(gpu, args):
    ...
    print(dataset_list,dataset_list_test)
    if parallel:
        train_sampler = torch.utils.data.distributed.DistributedSampler(
            dataset_list,num_replicas=args.world_size,rank=rank)
    else:
        train_sampler = None
    ...
    if parallel:
        test_sampler = torch.utils.data.distributed.DistributedSampler(
            dataset_list_test,num_replicas=args.world_size,rank=rank)
