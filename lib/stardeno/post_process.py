...
def default_process(vid,demosaic):
    if demosaic: plist = [bayer_bilinear,]
    else: plist = []
    plist += [white_balance, clip, gamma]
    pp_vid = rearrange(vid,'t c h w -> t h w c').cpu().numpy()
    pp_vid = process(pp_vid,plist)
    if pp_vid.ndim == 3: pp_vid = pp_vid[None,:]
    pp_vid = rearrange(pp_vid,'t h w c -> t c h w')
    pp_vid = exposure.equalize_hist(pp_vid)
    return pp_vid
