...
def save_video(burst,root,name,fstart=0,div=None,fmt="image"):
    if burst.ndim == 4:
        return save_video_single(burst,root,name,fstart,div,fmt)
    elif burst.ndim == 5:
        return save_video_batch(burst,root,name,fstart,div,fmt)
    else:
        raise ValueError(f"Uknown ndim case [{burst.ndim}]")

def save_video_batch(burst,root,name,fstart=0,div=None,fmt="image"):
    fns = []
    B = burst.shape[0]
    for b in range(B):
        name_b = "%s_%d" % (name,b)
        fn = save_video_single(burst[b],root,name_b,fstart,div,fmt)
        fns += fn
    print(fns)
    return fns

def save_video_single(burst,root,name,fstart=0,div=None,fmt="image"):

    # -- path --
    root = Path(str(root))
    if not root.exists():
        print(f"Making dir for save_video [{str(root)}]")
